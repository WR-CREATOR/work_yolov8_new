...
def make_anchors(feats, strides, grid_cell_offset=0.5):
    """Generate anchors from features.
            ...
            输出 anchor_points 为坐标点的集合，包括H,W 对应的所有坐标。
            该函数根据输入特征图生成了用于检测任务的锚点坐标及其对应的步长。
    """
    anchor_points, stride_tensor = [], []
    assert feats is not None
    dtype, device = feats[0].dtype, feats[0].device
    for i, stride in enumerate(strides):
        _, _, h, w = feats[i].shape # (N, reg_max*4+nc, H, W)
        sx = torch.arange(end=w, device=device, dtype=dtype) + grid_cell_offset  # shift x
        sy = torch.arange(end=h, device=device, dtype=dtype) + grid_cell_offset  # shift y
        sy, sx = torch.meshgrid(sy, sx, indexing="ij") #if TORCH_1_10 else torch.meshgrid(sy, sx)
        anchor_points.append(torch.stack((sx, sy), -1).view(-1, 2)) # (H*W, 2)，表示所有网格点的坐标。
        stride_tensor.append(torch.full((h * w, 1), stride, dtype=dtype, device=device))    # (H*W, 1)，存储了对应特征层上每个网格点的步长信息。
...
def dist2bbox(distance, anchor_points, xywh=True, dim=-1):
    ...
    distance （batch, anchors, 4)
    anchor_points形状为(总网格数, 2)
    ...
    return torch.cat((x1y1, x2y2), dim)  # xyxy bbox    (batch, anchors, 4)
